                ...
                <div className="bg-purple-700 p-20 my-10 text-white">
                    ...
                    <div className="flex flex-col gap-2 mt-4 mb-4">
                        {booked.availableSlots.map((slot, index) => (
                            <Link key={index} className="px-4 py-2 border border-gray-300 rounded-md hover:bg-blue-500 text-sm text-white" to={`/trainer-booked/${booked._id}`}>  <button
                    ...
                    <Link to={`/payment/${booked._id}`}><Button className="my-4" color="info">Join Now</Button></Link>
                </div>
                <p className="flex gap-10 my-10 ">{booked.packages.map((packag, ind) => <Card key={ind} className='max-w-sm'>
